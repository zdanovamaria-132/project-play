    ...
    def render(self, screen):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                screen.fill(colors[self.get_title_id((x, y))], rect)

    def get_title_id(self, position):
        return self.map[position[1]][position[0]]

    def is_free(self, position):
        return self.get_title_id(position) in self.free_tile
...
class Enemy:
    ...
    def render(self, screen):
        center = self.x * TILE_SIZE + TILE_SIZE // 2, self.y * TILE_SIZE + TILE_SIZE // 2
        pygame.draw.circle(screen, (150, 200, 100), center, TILE_SIZE // 2)
...
def main():
    ...
    while running:
        ...
        if game.check_win():
            game_over = True
            show_mess(screen, 'You won!')
        if game.check_lose():
            game_over = True
            show_mess(screen, 'You lost!')
        pygame.display.flip()
        clock.tick(FPS)
    pygame.quit()
